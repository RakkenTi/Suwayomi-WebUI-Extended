...
- (**Download**) Automatically fix broken downloads whose chapter no longer exists at the source (e.g. due to a re-upload): the chapter list gets refreshed and the replacement chapter gets downloaded instead (toggleable in the download settings, enabled by default)
...
- (**Migration**) Add button to migrate the entries of all dead sources at once (sources flagged by the health check or with obsolete/uninstalled extensions, entries of all of them get bulk migrated in a single run with manual review for entries without a confident match)
...
- (**Migration**) Fix bulk search stopping before all sources were searched in case a match got found but did not fulfill the search criteria (caused entries to require manual review even though a fitting match existed in a lower priority source)
